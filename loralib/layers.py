...
class LoRALayer:
    ...
    def register_lora_param(self):
        ...
        for param_name, lora_name in self.params_with_lora.items():
            ...
            self.register_parameter(
                f"{lora_name}_lora_b",
                nn.Parameter(
                    eval(f"self.{lora_name}_lora_B").new_zeros(
                        (eval(f"self.{lora_name}_lora_B").size(0))
                    )
                ),
            )
    ...
    def get_lora_bias(self, param_name: str):
        lora_name = self.params_with_lora[param_name]
        return eval(f"self.{lora_name}_lora_b")
...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            ...
            bias = self.get_lora_bias("weight").unsqueeze(0).unsqueeze(0)
            bias = bias.expand(
                lora_adjustment.shape[0],
                lora_adjustment.shape[1],
                lora_adjustment.shape[2],
            )
            result = original_output + lora_adjustment + bias
        else:
